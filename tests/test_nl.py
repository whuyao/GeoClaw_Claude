...
def test_n01_proc_rule_mode():
    """强制规则模式初始化"""
    p = NLProcessor(use_ai=False)
    assert not p._use_ai
def test_n02_proc_auto_no_key():
    """无任何有效 provider 时自动降级为规则模式"""
    import unittest.mock as mock
    with mock.patch("geoclaw_claude.nl.llm_provider.LLMProvider") as MockLLM:
        MockLLM.from_config.return_value = None
        p = NLProcessor(use_ai=None)
    assert not p._use_ai
def test_n03_proc_ai_flag():
    """强制 AI 模式时 _use_ai=True（即使 key 可能无效）"""
    p = NLProcessor(api_key="fake_key", use_ai=True)
    assert p._use_ai
def test_n04_proc_empty_input():
...
def test_n15_parsed_intent_fields():
    """ParsedIntent 字段完整性"""
    intent = ParsedIntent(
        action="buffer",
...
def test_n16_parsed_intent_multistep():
    """ParsedIntent 多步检测"""
...
def test_n17_parsed_intent_to_dict():
