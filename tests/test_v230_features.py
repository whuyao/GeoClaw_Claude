...
test("F01 token 估算 - 中英文", f01_token_estimate)


def f02_messages_token_estimate():
...
test("F02 消息列表 token 估算", f02_messages_token_estimate)


def f03_compress_not_needed():
...
test("F03 短对话无需压缩", f03_compress_not_needed)


def f04_compress_level1_summary():
    from geoclaw_claude.nl.context_compress import ContextCompressor, CompressConfig
    # 生成足够多消息触发 Level 1
    msgs = []
    for i in range(20):
...
test("F04 Level 1 摘要旧轮次", f04_compress_level1_summary)


def f05_compress_level2_dedup():
    from geoclaw_claude.nl.context_compress import ContextCompressor, CompressConfig
    msgs = [
...
test("F05 Level 2 语义去重", f05_compress_level2_dedup)


def f06_compress_level3_truncate():
    from geoclaw_claude.nl.context_compress import ContextCompressor, CompressConfig
...
test("F06 Level 3 强制截断", f06_compress_level3_truncate)


def f07_compress_ratio():
...
test("F07 压缩比例计算", f07_compress_ratio)


def f08_compress_convenience_fn():
...
test("F08 compress_if_needed 便捷函数", f08_compress_convenience_fn)


def f09_compress_local_summary_content():
...
test("F09 本地摘要生成内容合理", f09_compress_local_summary_content)


def f10_compress_similarity():
    from geoclaw_claude.nl.context_compress import ContextCompressor
    c = ContextCompressor()
    assert c._similarity("hello world", "hello world") > 0.9
    assert c._similarity("apple", "banana") < 0.5
    assert c._similarity("", "") == 0.0
test("F10 相似度计算", f10_compress_similarity)
...
def f11_provider_config_anthropic():
    ...
    assert cfg.is_valid
test("F11 ProviderConfig Anthropic 构造", f11_provider_config_anthropic)


def f12_provider_config_qwen_default_url():
    ...
    assert cfg.model == "qwen-plus"  # 默认模型
test("F12 Qwen Provider 自动填充 base_url 和默认模型", f12_provider_config_qwen_default_url)


def f13_provider_config_invalid():
...
test("F13 无 API Key 的 ProviderConfig.is_valid=False", f13_provider_config_invalid)


def f14_provider_default_models():
...
test("F14 三个 Provider 默认模型已定义", f14_provider_default_models)


def f15_llm_provider_from_config_no_key():
...
test("F15 from_config 无 Key 返回 None 或实例", f15_llm_provider_from_config_no_key)


def f16_llm_provider_chat_invalid_provider():
...
test("F16 未知 Provider 返回 None", f16_llm_provider_chat_invalid_provider)


def f17_clean_json_response():
    from geoclaw_claude.nl.llm_provider import clean_json_response
    raw = '```json\n{"action": "buffer"}\n```'
    cleaned = clean_json_response(raw)
    assert cleaned == '{"action": "buffer"}'
test("F17 clean_json_response 去除 markdown 代码块", f17_clean_json_response)


def f18_parse_json_response_valid():
...
test("F18 parse_json_response 解析有效 JSON", f18_parse_json_response_valid)


def f19_parse_json_response_invalid():
    from geoclaw_claude.nl.llm_provider import parse_json_response
    result = parse_json_response("这不是 JSON")
    assert result is None
test("F19 parse_json_response 无效 JSON 返回 None", f19_parse_json_response_invalid)


def f20_llm_response_bool():
    ...
    assert bool(r1)
    assert not bool(r2)
test("F20 LLMResponse bool 判断", f20_llm_response_bool)
...
def f21_safe_output_path_basic():
    ...
    try:
        ...
        shutil.rmtree(tmp)
test("F21 safe_output_path 在 output_dir 下", f21_safe_output_path_basic)


def f22_safe_output_path_auto_rename():
    guard, tmp, out, inp = _make_guard()
    try:
        # 创建同名文件
        ...
        shutil.rmtree(tmp)
test("F22 safe_output_path 自动序号避免覆盖", f22_safe_output_path_auto_rename)


def f23_check_write_blocks_input():
    ...
    try:
        ...
        shutil.rmtree(tmp)
test("F23 check_write 拒绝写入输入目录", f23_check_write_blocks_input)


def f24_check_write_allows_output():
    guard, tmp, out, inp = _make_guard()
    try:
        ...
        shutil.rmtree(tmp)
test("F24 check_write 允许写入 output_dir", f24_check_write_allows_output)


def f25_check_write_blocks_system():
    from geoclaw_claude.security import SecurityError
    guard, tmp, out, inp = _make_guard()
    try:
        try:
        ...
        shutil.rmtree(tmp)
test("F25 check_write 拒绝写入系统目录", f25_check_write_blocks_system)


def f26_check_write_blocks_path_traversal():
    from geoclaw_claude.security import SecurityError
    guard, tmp, out, inp = _make_guard()
    try:
        try:
        ...
        shutil.rmtree(tmp)
test("F26 check_write 拒绝路径穿越 ../ 攻击", f26_check_write_blocks_path_traversal)


def f27_check_delete_blocks_input():
    from geoclaw_claude.security import SecurityError
    guard, tmp, out, inp = _make_guard()
    try:
        ...
        shutil.rmtree(tmp)
test("F27 check_delete 拒绝删除输入文件", f27_check_delete_blocks_input)


def f28_check_delete_blocks_output_root():
    from geoclaw_claude.security import SecurityError
    guard, tmp, out, inp = _make_guard()
    try:
        try:
        ...
        shutil.rmtree(tmp)
test("F28 check_delete 禁止删除 output_dir 根目录", f28_check_delete_blocks_output_root)


def f29_is_input_file():
    guard, tmp, out, inp = _make_guard()
    try:
        ...
        shutil.rmtree(tmp)
test("F29 is_input_file 正确识别输入/输出文件", f29_is_input_file)


def f30_redirect_to_output():
    guard, tmp, out, inp = _make_guard()
    try:
        ...
        shutil.rmtree(tmp)
test("F30 redirect_to_output 正确重定向路径", f30_redirect_to_output)
...
test("F31 版本号 v2.3.0", f31_version)
...
def f32_nlprocessor_provider_param():
...
test("F32 NLProcessor provider 参数兼容性", f32_nlprocessor_provider_param)
...
test("F33 SecurityGuard.from_config() 正常初始化", f33_guard_from_config)
...
print("\n" + "═" * 54)
print(f"  v2.3.0 新功能测试: {_passed}/{total} 通过")
print("═" * 54)
...
if _failed:
    ...
    print("✅ 全部通过！上下文压缩 / 多 Provider / 安全机制均正常。")
