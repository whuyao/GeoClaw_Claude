...
def test_u02_version_compare():
    ...
    assert v110 <= v111
    assert not (v200 <= v100)
def test_u03_version_str():
    v = VersionInfo.parse("1.2.3")
    assert str(v) == "1.2.3"
def test_u04_version_edge_cases():
...
def test_u08_check_returns_result():
...
def test_u11_self_check_structure():
    """self_check() 报告结构完整。"""
    report = self_check()
...
def test_u12_self_check_modules():
...
def test_u13_print_self_check_no_crash():
...
def test_u14_changelog_diff_returns_str():
    """changelog_diff() 必须返回字符串，无论网络是否可用。"""
    result = changelog_diff("0.0.0")
    assert isinstance(result, str)
    # 有内容（非空 或 说明无法获取）
    assert len(result) > 0
def test_u15_changelog_diff_filter():
...
def test_u17_update_already_latest():
...
def test_u18_update_result_dataclass():
...
def test_u20_version():
    ...
    assert _get_local_version() == geoclaw_claude.__version__
# ════════════════════════════════════════════════════════════
#  汇总
# ════════════════════════════════════════════════════════════
