...
```bash
# 进入项目目录
cd /Volumes/Yaoy/project/Claud-Projects/GeoClaw-Claude
...
```python
import sys
sys.path.insert(0, "/Volumes/Yaoy/project/Claud-Projects/GeoClaw-Claude")
...
```bash
export PYTHONPATH="/Volumes/Yaoy/project/Claud-Projects/GeoClaw-Claude:$PYTHONPATH"
```

**方式三：开发模式安装（需要 setup.py，v0.2 计划添加）**

```bash
# 项目根目录下运行（setup.py 完成后可用）
pip install -e .
```
